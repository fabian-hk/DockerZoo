...
docker buildx build $1 --push --platform linux/amd64 -t fabianhk/btrbk-server:amd64-${DEBIAN_TAG} -f btrbk-server/Dockerfile .
docker buildx build $1 --push --platform linux/arm64 -t fabianhk/btrbk-server:arm64-${DEBIAN_TAG} -f btrbk-server/Dockerfile .
